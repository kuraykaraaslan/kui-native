// Zustand store backing the imperative `toast()` API and `useToast()` —
// ported verbatim from KuiReact's modules/ui/Toast/hooks/useToastStore.ts.
...
type ToastStore = {
  ...
  clear: () => void;
  /** Maximum concurrent toasts; the oldest are dropped past it (FIFO). */
  max: number;
...
export const useToastStore = create<ToastStore>((set) => ({
  ...
  max: 5,
  add: (item) => {
    // Hermes has no crypto.randomUUID; KuiReact's fallback id scheme works everywhere.
    counter += 1;
    const id = `toast-${Date.now()}-${counter}-${Math.random().toString(36).slice(2, 9)}`;
    set((s) => {
