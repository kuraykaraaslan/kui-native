...
> **Status: PARITY_COMPLETE** (was PARITY_MINOR_GAPS; error ring added in `02d510b`)
...
| Error look | `border-error ring-1 ring-error bg-error-subtle` | same (`ring-1` drawn as a 1px `error` outline, `02d510b`) | Match |
...
| Tests | 0 | 10 (the error case also checks the ring since `02d510b`) | Native-ahead |
