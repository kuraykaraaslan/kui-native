...
> Backlog item · KuiReact id `app-breadcrumbs` · layer `app` · **Navigation** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
**Why it matters for KuiNative:** Breadcrumbs with overflow collapse.

## KuiReact Reference

| | |
| --- | --- |
| Source | `kui-react/modules/app/AppBreadcrumbs.tsx` (1 file, 99 LOC) |
| Public export | `@kuraykaraaslan/kui-react/app` — public (npm: root + /app) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | With title + badge, Deep path |
| Composes | Breadcrumb (missing), Button (exists as `Button`), DropdownMenu (missing), Tooltip (missing) |
| Used by (registry) | — |
...
Parsed from `modules/app/AppBreadcrumbs.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `items` | `BreadcrumbItem[]` | no | `[]` |  | same |
...
- Breadcrumb — missing → [backlog](breadcrumb.md)
- Button — exists as `Button`
- DropdownMenu — missing → [backlog](dropdown-menu.md)
- Tooltip — missing → [backlog](tooltip.md)

**Blocked by (roadmap):** `breadcrumb`, `dropdown-menu`, `tooltip`, `R-button`

**Third-party:** `@fortawesome/react-fontawesome` → @fortawesome/react-native-fontawesome, `@fortawesome/free-solid-svg-icons`

**Suggested RN libraries:** `breadcrumb`

## Implementation Notes

Port with Breadcrumb.

- Location: `modules/app/AppBreadcrumbs.tsx`, named export `AppBreadcrumbs` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `AppBreadcrumbsProps` type.
- Add a showcase entry (`modules/showcase/registry.tsx`) with one demo per KuiReact variant.

## Acceptance Criteria

- [ ] Exported as `AppBreadcrumbs` from the `modules/app` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (With title + badge, Deep path)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `surface-raised`, `text-disabled`, `text-primary`, `text-secondary`); renders correctly in light and dark
