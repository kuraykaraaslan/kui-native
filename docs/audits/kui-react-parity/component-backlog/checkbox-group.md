...
> Backlog item · KuiReact id `checkbox-group` · layer `ui` · **Forms** · Priority **High** · Complexity **Small** · Wave 2 · Fit `direct` · est. 0.5–1 d
...
**Why it matters for KuiNative:** Composes Checkbox; needed for multi-choice forms and filter panels.

## KuiReact Reference

| | |
| --- | --- |
| Source | `kui-react/modules/ui/CheckboxGroup.tsx` (1 file, 67 LOC) |
| Public export | `@kuraykaraaslan/kui-react/ui` — public (npm: root + /ui) |
| Registry | yes · status `stable` · since 2025-02 |
| Showcase variants | Default, Disabled |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/ui/CheckboxGroup.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `legend` | `string` | **yes** | — |  | same |
| `options` | `CheckboxOption[]` | **yes** | — |  | same |
| `selected` | `string[]` | **yes** | — |  | same |
| `onChange` | `(next: string[]) => void` | **yes** | — |  | same |
...
- Location: `modules/ui/CheckboxGroup.tsx`, named export `CheckboxGroup` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CheckboxGroupProps` type.
- Add a showcase entry (`modules/showcase/registry.tsx`) with one demo per KuiReact variant.

## Acceptance Criteria

- [ ] Exported as `CheckboxGroup` from the `modules/ui` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Default, Disabled)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `primary`, `primary-subtle`, `surface-base`, `surface-overlay`, `text-primary`); renders correctly in light and dark
