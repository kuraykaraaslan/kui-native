...
> Backlog item · KuiReact id `blog-post-status-badge` · layer `domain` · **Domain — Blog** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/blog/post/PostStatusBadge.tsx` (1 file, 27 LOC) |
...
| Used by (registry) | — |
| Usage frequency | imported by 4 production file(s), 3 showcase file(s) |
| Third-party imports | — |
| Unit tests | none |
| Interaction flags | — |
...
Parsed from `modules/domains/blog/post/PostStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/blog/PostStatusBadge.tsx`, named export `PostStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
...
- [ ] Exported as `PostStatusBadge` from the `modules/domains/blog` barrel with its props type
