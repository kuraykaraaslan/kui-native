...
> Backlog item · KuiReact id `blog-author-bio-card` · layer `domain` · **Domain — Blog** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/blog/author/AuthorBioCard.tsx` (1 file, 139 LOC) |
| Public export | `@/modules/domains/blog/author/AuthorBioCard` — source-only (not exported from a barrel) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Verified author with socials, Minimal author with follow |
...
Parsed from `modules/domains/blog/author/AuthorBioCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `name` | `string` | **yes** | — |  | same |
| `username` | `string` | no | — |  | same |
| `avatar` | `string \| null` | no | — |  | same |
| `biography` | `string \| null` | no | — |  | same |
| `role` | `string` | no | — |  | same |
| `verified` | `boolean` | no | — |  | same |
| `joinedAt` | `Date \| string` | no | — |  | same |
| `socials` | `{ twitter?: string; linkedin?: string; github?: string; website?: string; email…` | no | — |  | same |
| `isFollowing` | `boolean` | no | — |  | same |
| `onFollow` | `() => void` | no | — |  | same |
...
- Location: `modules/domains/blog/AuthorBioCard.tsx`, named export `AuthorBioCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `AuthorBioCardProps` type.
...
- [ ] Exported as `AuthorBioCard` from the `modules/domains/blog` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Verified author with socials, Minimal author with follow)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `surface-raised`, `text-disabled`, `text-primary`, `text-secondary`); renders correctly in light and dark
