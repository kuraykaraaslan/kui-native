...
> Backlog item · KuiReact id `blog-post-card` · layer `domain` · **Domain — Blog** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/blog/post/PostCard.tsx` (1 file, 74 LOC) |
...
| Showcase variants | Default, With status badge |
| Composes | CategoryBadge (missing), PostMeta (missing), PostStatusBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/blog/post/PostCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `post` | `PostWithData` | **yes** | — |  | same |
| `showStatus` | `boolean` | no | `false` |  | same |
...
- CategoryBadge — missing → [backlog](blog-category-badge.md)
- PostMeta — missing → [backlog](blog-post-meta.md)
- PostStatusBadge — missing → [backlog](blog-post-status-badge.md)
...
- Location: `modules/domains/blog/PostCard.tsx`, named export `PostCard` (same name as KuiReact), re-exported from the layer barrel.
...
- [ ] Exported as `PostCard` from the `modules/domains/blog` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Default, With status badge)
