...
> Backlog item · KuiReact id `travel-flight-segment-status-badge` · layer `domain` · **Domain — Travel** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/travel/flight/FlightSegmentStatusBadge.tsx` (1 file, 27 LOC) |
...
Parsed from `modules/domains/travel/flight/FlightSegmentStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `FlightSegmentStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/travel/FlightSegmentStatusBadge.tsx`, named export `FlightSegmentStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `FlightSegmentStatusBadgeProps` type.
...
- [ ] Exported as `FlightSegmentStatusBadge` from the `modules/domains/travel` barrel with its props type
