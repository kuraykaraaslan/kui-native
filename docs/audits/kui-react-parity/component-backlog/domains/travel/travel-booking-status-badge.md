...
> Backlog item · KuiReact id `travel-booking-status-badge` · layer `domain` · **Domain — Travel** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/travel/booking/BookingStatusBadge.tsx` (1 file, 29 LOC) |
...
Parsed from `modules/domains/travel/booking/BookingStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `TravelBookingStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/travel/BookingStatusBadge.tsx`, named export `BookingStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `BookingStatusBadgeProps` type.
...
- [ ] Exported as `BookingStatusBadge` from the `modules/domains/travel` barrel with its props type
