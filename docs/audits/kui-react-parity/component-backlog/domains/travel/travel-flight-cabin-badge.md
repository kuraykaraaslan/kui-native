...
> Backlog item · KuiReact id `travel-flight-cabin-badge` · layer `domain` · **Domain — Travel** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/travel/flight/FlightCabinBadge.tsx` (1 file, 26 LOC) |
| Public export | `@/modules/domains/travel` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All cabins, Small size |
...
Parsed from `modules/domains/travel/flight/FlightCabinBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `cabin` | `FlightCabinClass` | **yes** | — |  | same |
...
- Location: `modules/domains/travel/FlightCabinBadge.tsx`, named export `FlightCabinBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `FlightCabinBadgeProps` type.
...
- [ ] Exported as `FlightCabinBadge` from the `modules/domains/travel` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All cabins, Small size)
