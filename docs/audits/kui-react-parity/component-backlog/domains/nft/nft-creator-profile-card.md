...
> Backlog item · KuiReact id `nft-creator-profile-card` · layer `domain` · **Domain — NFT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/nft/creator/CreatorProfileCard.tsx` (1 file, 143 LOC) |
| Public export | `@/modules/domains/nft` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Not following, Following |
...
Parsed from `modules/domains/nft/creator/CreatorProfileCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `creator` | `NftCreator` | **yes** | — |  | same |
| `following` | `boolean` | no | `false` |  | same |
| `onFollowToggle` | `() => void` | no | — |  | same |
...
- Location: `modules/domains/nft/CreatorProfileCard.tsx`, named export `CreatorProfileCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CreatorProfileCardProps` type.
...
- [ ] Exported as `CreatorProfileCard` from the `modules/domains/nft` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Not following, Following)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `surface-base`, `surface-overlay`, `surface-raised`, `surface-sunken`, `text-primary`, …); renders correctly in light and dark
