...
> Backlog item · KuiReact id `nft-blockchain-badge` · layer `domain` · **Domain — NFT** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/nft/wallet/BlockchainBadge.tsx` (1 file, 52 LOC) |
| Public export | `@/modules/domains/nft` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All chains, Icon-only |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/nft/wallet/BlockchainBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `chain` | `Blockchain` | **yes** | — |  | same |
...
| `aria-prohibited` | review manually |
...
- Location: `modules/domains/nft/BlockchainBadge.tsx`, named export `BlockchainBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `BlockchainBadgeProps` type.
...
- [ ] Exported as `BlockchainBadge` from the `modules/domains/nft` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All chains, Icon-only)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`info-fg`, `info-subtle`, `primary`, `primary-subtle`, `secondary`, `secondary-subtle`); renders correctly in light and dark
