...
> Backlog item · KuiReact id `nft-detail-header` · layer `domain` · **Domain — NFT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/nft/asset/NftDetailHeader.tsx` (1 file, 101 LOC) |
| Public export | `@/modules/domains/nft` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Full header |
| Composes | Avatar (exists as `Avatar` + `AvatarGroup`), BlockchainBadge (missing), RarityBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/nft/asset/NftDetailHeader.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `asset` | `NftAsset` | **yes** | — |  | same |
| `collectionName` | `string` | **yes** | — |  | same |
| `collectionSlug` | `string` | **yes** | — |  | same |
| `ownerAvatar` | `string` | no | — |  | same |
| `className` | `string` | no | — |  | keep (NativeWind) |
| `onLike` | `() => void` | no | — |  | same |
| `onShare` | `() => void` | no | — |  | same |
...
- RarityBadge — missing → [backlog](nft-rarity-badge.md)
...
- Location: `modules/domains/nft/NftDetailHeader.tsx`, named export `NftDetailHeader` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `NftDetailHeaderProps` type.
...
- [ ] Exported as `NftDetailHeader` from the `modules/domains/nft` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Full header)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `primary`, `surface-base`, `surface-overlay`, `text-primary`, `text-secondary`); renders correctly in light and dark
