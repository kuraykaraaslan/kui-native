...
> Backlog item · KuiReact id `nft-creator-leaderboard-row` · layer `domain` · **Domain — NFT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/nft/creator/CreatorLeaderboardRow.tsx` (1 file, 84 LOC) |
| Public export | `@/modules/domains/nft` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Top 3 leaderboard |
...
Parsed from `modules/domains/nft/creator/CreatorLeaderboardRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `rank` | `number` | **yes** | — |  | same |
...
- Location: `modules/domains/nft/CreatorLeaderboardRow.tsx`, named export `CreatorLeaderboardRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CreatorLeaderboardRowProps` type.
...
- [ ] Exported as `CreatorLeaderboardRow` from the `modules/domains/nft` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Top 3 leaderboard)
...
- [ ] Uses only semantic tokens (`border-focus`, `error`, `error-fg`, `error-subtle`, `primary`, `success`, `surface-overlay`, `surface-sunken`, …); renders correctly in light and dark
