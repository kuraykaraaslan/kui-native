...
> Backlog item · KuiReact id `nft-wallet-connect-button` · layer `domain` · **Domain — NFT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/nft/wallet/WalletConnectButton.tsx` (1 file, 134 LOC) |
| Public export | `@/modules/domains/nft` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Disconnected, Connected |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/nft/wallet/WalletConnectButton.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `wallet` | `ConnectedWallet \| null` | no | `null` |  | same |
| `onConnect` | `(provider: WalletProvider) => void` | no | — |  | same |
| `onDisconnect` | `() => void` | no | — |  | same |
...
- Location: `modules/domains/nft/WalletConnectButton.tsx`, named export `WalletConnectButton` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `WalletConnectButtonProps` type.
...
- [ ] Exported as `WalletConnectButton` from the `modules/domains/nft` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Disconnected, Connected)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `error-fg`, `error-subtle`, `info-fg`, `info-subtle`, `primary`, …); renders correctly in light and dark
