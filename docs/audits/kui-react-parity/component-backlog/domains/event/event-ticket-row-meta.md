...
> Backlog item · KuiReact id `event-ticket-row-meta` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/TicketRowMeta.tsx` (1 file, 97 LOC) |
| Public export | `@/modules/domains/event` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Default, Compact (vertical layout) |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/event/TicketRowMeta.tsx` (TicketRowMeta + TicketRowActions). **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `entry` | `TicketRowEntry` | **yes** | — |  | same |
| `compact` | `boolean` | no | — |  | same |
| `entry` | `TicketRowEntry` | **yes** | — | (TicketRowActions) | same |
| `compact` | `boolean` | no | — | (TicketRowActions) | same |
...
- Location: `modules/domains/event/TicketRowMeta.tsx`, named export `TicketRowMeta` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `TicketRowMetaProps` type.
...
- [ ] Exported as `TicketRowMeta` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Default, Compact (vertical layout))
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `error-subtle`, `info`, `info-subtle`, `success`, `success-subtle`, …); renders correctly in light and dark
