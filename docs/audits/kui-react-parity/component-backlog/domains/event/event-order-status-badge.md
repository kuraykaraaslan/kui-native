...
> Backlog item · KuiReact id `event-order-status-badge` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/EventOrderStatusBadge.tsx` (1 file, 34 LOC) |
| Public export | `@/modules/domains/event` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All statuses — sm, Size md (detail view) |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/event/EventOrderStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/event/EventOrderStatusBadge.tsx`, named export `EventOrderStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `EventOrderStatusBadgeProps` type.
...
- [ ] Exported as `EventOrderStatusBadge` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All statuses — sm, Size md (detail view))
- [ ] Uses only semantic tokens (`error`, `error-subtle`, `success`, `success-subtle`, `warning`, `warning-subtle`); renders correctly in light and dark
