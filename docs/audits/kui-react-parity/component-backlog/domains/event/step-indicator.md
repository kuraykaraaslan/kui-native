...
> Backlog item · KuiReact id `step-indicator` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/StepIndicator.tsx` (1 file, 58 LOC) |
...
| Showcase variants | Tüm adım durumları |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/event/StepIndicator.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `current` | `CheckoutStep` | **yes** | — |  | same |
...
| --- | --- | --- | --- |
| selected / active | `current` | highlighted | `accessibilityState.selected` |
...
- Location: `modules/domains/event/StepIndicator.tsx`, named export `StepIndicator` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `StepIndicatorProps` type.
...
- [ ] Exported as `StepIndicator` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Tüm adım durumları)
- [ ] State **selected / active** implemented: `accessibilityState.selected`
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`border`, `primary`, `primary-fg`, `success`, `surface-overlay`, `text-disabled`, `text-secondary`); renders correctly in light and dark
