...
> Backlog item · KuiReact id `event-card` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/EventCard.tsx` (1 file, 93 LOC) |
...
| Showcase variants | Yayında, Satışı Bitti |
| Composes | EventCategoryBadge (missing), EventFormatBadge (missing), EventStatusBadge (missing) |
| Used by (registry) | — |
| Usage frequency | imported by 9 production file(s), 1 showcase file(s) |
...
Parsed from `modules/domains/event/EventCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `event` | `EventWithData` | **yes** | — |  | same |
...
- EventCategoryBadge — missing → [backlog](event-category-badge.md)
- EventFormatBadge — missing → [backlog](event-format-badge.md)
- EventStatusBadge — missing → [backlog](event-status-badge.md)
...
- Location: `modules/domains/event/EventCard.tsx`, named export `EventCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `EventCardProps` type.
...
- [ ] Exported as `EventCard` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Yayında, Satışı Bitti)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `border-strong`, `primary`, `surface-overlay`, `surface-raised`, `text-disabled`, `text-primary`, …); renders correctly in light and dark
