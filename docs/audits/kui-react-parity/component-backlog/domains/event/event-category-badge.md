...
> Backlog item · KuiReact id `event-category-badge` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/EventCategoryBadge.tsx` (1 file, 34 LOC) |
...
| Showcase variants | Badge ve link |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/event/EventCategoryBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `category` | `Pick<EventCategory, 'title' \| 'slug' \| 'image'>` | **yes** | — |  | same |
...
- Location: `modules/domains/event/EventCategoryBadge.tsx`, named export `EventCategoryBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `EventCategoryBadgeProps` type.
...
- [ ] Exported as `EventCategoryBadge` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Badge ve link)
- [ ] State **hover** implemented: pressed state instead (`active:` classes / Pressable `pressed`)
- [ ] Uses only semantic tokens (`border`, `surface-overlay`, `surface-sunken`, `text-primary`, `text-secondary`); renders correctly in light and dark
