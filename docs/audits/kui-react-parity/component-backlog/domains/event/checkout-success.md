...
> Backlog item · KuiReact id `checkout-success` · layer `domain` · **Domain — Event** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/event/CheckoutSuccess.tsx` (1 file, 122 LOC) |
...
| Showcase variants | Başarı ekranı |
| Composes | Button (exists as `Button`), TicketCard (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/event/CheckoutSuccess.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `orderId` | `string` | **yes** | — |  | same |
| `ticketId` | `string` | **yes** | — |  | same |
| `buyerName` | `string` | **yes** | — |  | same |
| `buyerEmail` | `string` | **yes** | — |  | same |
| `event` | `{ eventId: string; title: string; startAt: Date; slug: string; }` | **yes** | — |  | same |
| `venue` | `{ name: string; city: string }` | **yes** | — |  | same |
| `cartItems` | `CartItem[]` | **yes** | — |  | same |
| `total` | `number` | **yes** | — |  | same |
...
- TicketCard — missing → [backlog](ticket-card.md)
...
- Location: `modules/domains/event/CheckoutSuccess.tsx`, named export `CheckoutSuccess` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CheckoutSuccessProps` type.
...
- [ ] Exported as `CheckoutSuccess` from the `modules/domains/event` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Başarı ekranı)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-fg`, `primary-hover`, `success`, `success-subtle`, `surface-overlay`, …); renders correctly in light and dark
