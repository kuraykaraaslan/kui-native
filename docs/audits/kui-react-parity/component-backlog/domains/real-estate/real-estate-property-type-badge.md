...
> Backlog item · KuiReact id `real-estate-property-type-badge` · layer `domain` · **Domain — Real Estate** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/real-estate/property/PropertyTypeBadge.tsx` (1 file, 20 LOC) |
...
Parsed from `modules/domains/real-estate/property/PropertyTypeBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `type` | `PropertyType` | **yes** | — |  | same |
...
- Location: `modules/domains/real-estate/PropertyTypeBadge.tsx`, named export `PropertyTypeBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `PropertyTypeBadgeProps` type.
...
- [ ] Exported as `PropertyTypeBadge` from the `modules/domains/real-estate` barrel with its props type
