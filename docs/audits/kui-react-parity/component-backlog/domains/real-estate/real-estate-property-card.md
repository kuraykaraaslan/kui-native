...
> Backlog item · KuiReact id `real-estate-property-card` · layer `domain` · **Domain — Real Estate** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/real-estate/property/PropertyCard.tsx` (1 file, 127 LOC) |
| Public export | `@/modules/domains/real-estate` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | For sale with image, Rental — no image (gradient placeholder), Short-term villa |
| Composes | ListingTypeBadge (missing), PropertyTypeBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/real-estate/property/PropertyCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `property` | `PropertyCardData` | **yes** | — |  | same |
...
- ListingTypeBadge — missing → [backlog](real-estate-listing-type-badge.md)
- PropertyTypeBadge — missing → [backlog](real-estate-property-type-badge.md)
...
- Location: `modules/domains/real-estate/PropertyCard.tsx`, named export `PropertyCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `PropertyCardProps` type.
...
- [ ] Exported as `PropertyCard` from the `modules/domains/real-estate` barrel with its props type
...
- [ ] Showcase demos for all 3 KuiReact variants (For sale with image, Rental — no image (gradient placeholder), Short-term villa)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-subtle`, `surface-overlay`, `surface-raised`, `surface-sunken`, `text-disabled`, …); renders correctly in light and dark
