...
> Backlog item · KuiReact id `media-video-meta` · layer `domain` · **Domain — Media** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/media/video/VideoMeta.tsx` (1 file, 69 LOC) |
| Public export | `@/modules/domains/media` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Full meta, Views and date only |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/media/video/VideoMeta.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `video` | `{ viewCount: number; likeCount?: number; publishedAt?: Date \| string \| null; du…` | **yes** | — |  | same |
...
- Location: `modules/domains/media/VideoMeta.tsx`, named export `VideoMeta` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `VideoMetaProps` type.
...
- [ ] Exported as `VideoMeta` from the `modules/domains/media` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Full meta, Views and date only)
