...
> Backlog item · KuiReact id `media-video-card` · layer `domain` · **Domain — Media** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/media/video/VideoCard.tsx` (1 file, 146 LOC) |
| Public export | `@/modules/domains/media` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Published with thumbnail, Draft, no thumbnail |
...
Parsed from `modules/domains/media/video/VideoCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `video` | `VideoCardVideo` | **yes** | — |  | same |
...
- Location: `modules/domains/media/VideoCard.tsx`, named export `VideoCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `VideoCardProps` type.
...
- [ ] Exported as `VideoCard` from the `modules/domains/media` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Published with thumbnail, Draft, no thumbnail)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `info`, `surface-overlay`, `surface-raised`, `surface-sunken`, `text-disabled`, `text-primary`, …); renders correctly in light and dark
