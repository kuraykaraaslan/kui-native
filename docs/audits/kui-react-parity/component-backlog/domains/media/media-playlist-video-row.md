...
> Backlog item · KuiReact id `media-playlist-video-row` · layer `domain` · **Domain — Media** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/media/playlist/PlaylistVideoRow.tsx` (1 file, 106 LOC) |
| Public export | `@/modules/domains/media` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Now playing + idle, Draggable (reorderable) |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/media/playlist/PlaylistVideoRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `position` | `number` | **yes** | — |  | same |
| `title` | `string` | **yes** | — |  | same |
| `channelName` | `string` | **yes** | — |  | same |
| `channelVerified` | `boolean` | no | — |  | same |
| `thumbnailUrl` | `string \| null` | no | — |  | same |
| `duration` | `number` | no | — |  | same |
| `isPlaying` | `boolean` | no | — |  | same |
| `draggable` | `boolean` | no | — |  | same |
...
- Location: `modules/domains/media/PlaylistVideoRow.tsx`, named export `PlaylistVideoRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `PlaylistVideoRowProps` type.
...
- [ ] Exported as `PlaylistVideoRow` from the `modules/domains/media` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Now playing + idle, Draggable (reorderable))
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `info`, `primary`, `primary-subtle`, `surface-overlay`, `surface-sunken`, `text-disabled`, …); renders correctly in light and dark
