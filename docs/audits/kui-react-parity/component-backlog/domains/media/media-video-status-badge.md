...
> Backlog item · KuiReact id `media-video-status-badge` · layer `domain` · **Domain — Media** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/media/video/VideoStatusBadge.tsx` (1 file, 29 LOC) |
...
Parsed from `modules/domains/media/video/VideoStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/media/VideoStatusBadge.tsx`, named export `VideoStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `VideoStatusBadgeProps` type.
...
- [ ] Exported as `VideoStatusBadge` from the `modules/domains/media` barrel with its props type
