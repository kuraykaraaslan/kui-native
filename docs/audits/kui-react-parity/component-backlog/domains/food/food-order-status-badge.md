...
> Backlog item · KuiReact id `food-order-status-badge` · layer `domain` · **Domain — Food** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/food/order/OrderStatusBadge.tsx` (1 file, 33 LOC) |
| Public export | `@/modules/domains/food` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All statuses, Key states |
...
Parsed from `modules/domains/food/order/OrderStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `OrderStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/food/OrderStatusBadge.tsx`, named export `OrderStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `OrderStatusBadgeProps` type.
...
- [ ] Exported as `OrderStatusBadge` from the `modules/domains/food` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All statuses, Key states)
