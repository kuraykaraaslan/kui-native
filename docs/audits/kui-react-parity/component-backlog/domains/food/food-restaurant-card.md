...
> Backlog item · KuiReact id `food-restaurant-card` · layer `domain` · **Domain — Food** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/food/restaurant/RestaurantCard.tsx` (1 file, 134 LOC) |
| Public export | `@/modules/domains/food` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Active with link, Closed restaurant |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/food/restaurant/RestaurantCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `restaurant` | `{ restaurantId: string; name: string; slug: string; cuisineTypes: string[]; rat…` | **yes** | — |  | same |
...
- Location: `modules/domains/food/RestaurantCard.tsx`, named export `RestaurantCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `RestaurantCardProps` type.
...
- [ ] Exported as `RestaurantCard` from the `modules/domains/food` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Active with link, Closed restaurant)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `border-strong`, `primary`, `secondary`, `success`, `surface-base`, `surface-raised`, …); renders correctly in light and dark
