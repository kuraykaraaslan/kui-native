...
> Backlog item · KuiReact id `food-cuisine-tag-chip` · layer `domain` · **Domain — Food** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/food/cuisine/CuisineTagChip.tsx` (1 file, 70 LOC) |
| Public export | `@/modules/domains/food` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Selected + neutral, Without counts |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/food/cuisine/CuisineTagChip.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `label` | `string` | **yes** | — |  | same |
| `selected` | `boolean` | no | — |  | same |
...
- Location: `modules/domains/food/CuisineTagChip.tsx`, named export `CuisineTagChip` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CuisineTagChipProps` type.
...
- [ ] Exported as `CuisineTagChip` from the `modules/domains/food` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Selected + neutral, Without counts)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-fg`, `primary-hover`, `surface-base`, `surface-overlay`, `text-primary`, …); renders correctly in light and dark
