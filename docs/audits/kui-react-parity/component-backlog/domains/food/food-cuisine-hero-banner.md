...
> Backlog item · KuiReact id `food-cuisine-hero-banner` · layer `domain` · **Domain — Food** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/food/cuisine/CuisineHeroBanner.tsx` (1 file, 125 LOC) |
| Public export | `@/modules/domains/food` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Italian, No image, gradient fallback |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/food/cuisine/CuisineHeroBanner.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `cuisine` | `string` | **yes** | — |  | same |
| `description` | `string` | no | — |  | same |
| `icon` | `CuisineIconKey` | no | `'default'` |  | same |
| `restaurantCount` | `number` | no | — |  | same |
| `averageRating` | `number` | no | — |  | same |
| `averageDeliveryMin` | `number` | no | — |  | same |
...
- Location: `modules/domains/food/CuisineHeroBanner.tsx`, named export `CuisineHeroBanner` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CuisineHeroBannerProps` type.
...
- [ ] Exported as `CuisineHeroBanner` from the `modules/domains/food` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Italian, No image, gradient fallback)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`border`, `primary`, `primary-hover`, `secondary`); renders correctly in light and dark
