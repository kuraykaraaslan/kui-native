...
> Backlog item · KuiReact id `food-delivery-status-badge` · layer `domain` · **Domain — Food** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/food/order/DeliveryStatusBadge.tsx` (1 file, 30 LOC) |
...
Parsed from `modules/domains/food/order/DeliveryStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `DeliveryStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/food/DeliveryStatusBadge.tsx`, named export `DeliveryStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `DeliveryStatusBadgeProps` type.
...
- [ ] Exported as `DeliveryStatusBadge` from the `modules/domains/food` barrel with its props type
