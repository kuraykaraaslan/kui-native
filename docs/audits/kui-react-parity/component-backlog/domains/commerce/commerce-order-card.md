...
> Backlog item · KuiReact id `commerce-order-card` · layer `domain` · **Domain — Commerce** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/commerce/order/OrderCard.tsx` (1 file, 94 LOC) |
| Public export | `@/modules/domains/commerce` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Delivered order with link, Pending order (no link) |
| Composes | OrderStatusBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/commerce/order/OrderCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `order` | `OrderCardOrder` | **yes** | — |  | same |
...
- OrderStatusBadge — missing → [backlog](commerce-order-status-badge.md)
...
- Location: `modules/domains/commerce/OrderCard.tsx`, named export `OrderCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `OrderCardProps` type.
...
- [ ] Exported as `OrderCard` from the `modules/domains/commerce` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Delivered order with link, Pending order (no link))
