...
> Backlog item · KuiReact id `commerce-cart-item` · layer `domain` · **Domain — Commerce** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/commerce/cart/CartItem.tsx` (1 file, 79 LOC) |
| Public export | `@/modules/domains/commerce` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Single item with remove, Multi-quantity item |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/commerce/cart/CartItem.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `item` | `CartItemData` | **yes** | — |  | same |
...
- Location: `modules/domains/commerce/CartItem.tsx`, named export `CartItem` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CartItemProps` type.
...
- [ ] Exported as `CartItem` from the `modules/domains/commerce` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Single item with remove, Multi-quantity item)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `error-subtle`, `surface-raised`, `surface-sunken`, `text-disabled`, `text-primary`, …); renders correctly in light and dark
