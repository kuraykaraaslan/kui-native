...
> Backlog item · KuiReact id `commerce-order-status-badge` · layer `domain` · **Domain — Commerce** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/commerce/order/OrderStatusBadge.tsx` (1 file, 31 LOC) |
...
Parsed from `modules/domains/commerce/order/OrderStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/commerce/OrderStatusBadge.tsx`, named export `OrderStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
...
- [ ] Exported as `OrderStatusBadge` from the `modules/domains/commerce` barrel with its props type
