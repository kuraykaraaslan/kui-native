...
> Backlog item · KuiReact id `common-country-selector` · layer `domain` · **Domain — Common** · Priority **Medium** · Complexity **Large** · Wave 3 · Fit `adapt` · est. 5–8 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/common/location/CountrySelector.tsx` (1 file, 221 LOC) |
| Public export | `@kuraykaraaslan/kui-react/common` — public (npm: root + /common) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Default, With hint & error, No label |
| Composes | Button (exists as `Button`) |
| Used by (registry) | — |
| Usage frequency | imported by 2 production file(s), 1 showcase file(s) |
| Third-party imports | `react-dom`, `countries-list`, `country-flag-icons/react/3x2`, `@fortawesome/react-fontawesome`, `@fortawesome/free-solid-svg-icons` |
| Unit tests | none |
| Interaction flags | focus-visible ring, sr-only text, portal, animated/transitions, hover styles |

## Required Props

Parsed from `modules/domains/common/location/CountrySelector.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `value` | `string` | **yes** | — |  | same |
| `onChange` | `(iso2: string) => void` | **yes** | — |  | same |
| `id` | `string` | no | — |  | → `nativeID` / `testID` (RN needs no id for label wiring) |
| `label` | `string` | no | `'Country'` |  | same |
| `placeholder` | `string` | no | `'Select country…'` |  | same |
...
**Third-party:** `react-dom` → root overlay host / RN Modal, `countries-list`, `country-flag-icons/react/3x2`, `@fortawesome/react-fontawesome` → @fortawesome/react-native-fontawesome, `@fortawesome/free-solid-svg-icons`

## Implementation Notes

- Location: `modules/domains/common/CountrySelector.tsx`, named export `CountrySelector` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CountrySelectorProps` type.
...
- [ ] Exported as `CountrySelector` from the `modules/domains/common` barrel with its props type
...
- [ ] Showcase demos for all 3 KuiReact variants (Default, With hint & error, No label)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `primary`, `primary-subtle`, `surface-base`, `surface-overlay`, `surface-raised`, …); renders correctly in light and dark
