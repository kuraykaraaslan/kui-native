...
> Backlog item · KuiReact id `common-seo-form` · layer `domain` · **Domain — Common** · Priority **Medium** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/common/seo/SeoForm.tsx` (1 file, 93 LOC) |
...
| Composes | Button (exists as `Button`), Form (missing), Input (exists as `TextInput`), TagInput (missing), Textarea (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/common/seo/SeoForm.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `initial` | `Partial<SeoFields>` | no | `{}` |  | same |
| `onSubmit` | `(values: SeoFields) => Promise<void> \| void` | **yes** | — |  | same |
...
- Location: `modules/domains/common/SeoForm.tsx`, named export `SeoForm` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `SeoFormProps` type.
...
- [ ] Exported as `SeoForm` from the `modules/domains/common` barrel with its props type
