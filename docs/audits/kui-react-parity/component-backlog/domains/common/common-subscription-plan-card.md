...
> Backlog item · KuiReact id `common-subscription-plan-card` · layer `domain` · **Domain — Common** · Priority **Medium** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/common/subscription/SubscriptionPlanCard.tsx` (1 file, 122 LOC) |
| Public export | `@kuraykaraaslan/kui-react/common` — public (npm: root + /common) |
| Registry | yes · status `stable` · since 2025-05 |
| Showcase variants | Plan grid, Single card states |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/common/subscription/SubscriptionPlanCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `plan` | `SubscriptionPlan` | **yes** | — |  | same |
| `isCurrent` | `boolean` | no | `false` |  | same |
| `isSelected` | `boolean` | no | `false` |  | same |
| `onSelect` | `(planId: string) => void` | no | — |  | same |
...
- Location: `modules/domains/common/SubscriptionPlanCard.tsx`, named export `SubscriptionPlanCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `SubscriptionPlanCardProps` type.
...
- [ ] Exported as `SubscriptionPlanCard` from the `modules/domains/common` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Plan grid, Single card states)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-active`, `primary-fg`, `primary-hover`, `success`, `surface-raised`, …); renders correctly in light and dark
