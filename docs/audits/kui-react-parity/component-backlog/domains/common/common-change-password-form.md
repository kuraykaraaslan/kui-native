...
> Backlog item · KuiReact id `common-change-password-form` · layer `domain` · **Domain — Common** · Priority **Medium** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/common/auth/ChangePasswordForm.tsx` (1 file, 87 LOC) |
...
Parsed from `modules/domains/common/auth/ChangePasswordForm.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `onSubmit` | `(values: ChangePassword) => Promise<void> \| void` | **yes** | — |  | same |
...
- Location: `modules/domains/common/ChangePasswordForm.tsx`, named export `ChangePasswordForm` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `ChangePasswordFormProps` type.
...
- [ ] Exported as `ChangePasswordForm` from the `modules/domains/common` barrel with its props type
