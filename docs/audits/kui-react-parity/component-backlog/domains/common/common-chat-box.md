...
> Backlog item · KuiReact id `common-chat-box` · layer `domain` · **Domain — Common** · Priority **Medium** · Complexity **Large** · Wave 3 · Fit `adapt` · est. 5–8 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/common/chat/ChatBox.tsx` (1 file, 295 LOC) |
| Public export | `@kuraykaraaslan/kui-react/common` — public (npm: root + /common) |
| Registry | yes · status `beta` · since 2025-05 |
| Showcase variants | With initial messages, Empty / initial state |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/common/chat/ChatBox.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `title` | `string` | no | `'Support Chat'` |  | same |
| `subtitle` | `string` | no | `'We typically reply in a few …` |  | same |
| `placeholder` | `string` | no | `'Type a message…'` |  | same |
| `initialMessages` | `ChatMessage[]` | no | `[]` |  | same |
| `onSend` | `(text: string) => Promise<string> \| string` | no | — |  | same |
...
- Location: `modules/domains/common/ChatBox.tsx`, named export `ChatBox` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `ChatBoxProps` type.
...
- [ ] Exported as `ChatBox` from the `modules/domains/common` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (With initial messages, Empty / initial state)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `primary`, `primary-active`, `primary-fg`, `primary-hover`, `surface-base`, …); renders correctly in light and dark
