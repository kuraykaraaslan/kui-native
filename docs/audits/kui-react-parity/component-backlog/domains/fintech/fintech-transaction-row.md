...
> Backlog item · KuiReact id `fintech-transaction-row` · layer `domain` · **Domain — Fintech** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/fintech/transaction/TransactionRow.tsx` (1 file, 113 LOC) |
| Public export | `@/modules/domains/fintech` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Completed deposit, Mixed statuses |
| Composes | TransactionStatusBadge (missing), TransactionTypeBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/fintech/transaction/TransactionRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `transaction` | `Transaction` | **yes** | — |  | same |
...
- TransactionStatusBadge — missing → [backlog](fintech-transaction-status-badge.md)
- TransactionTypeBadge — missing → [backlog](fintech-transaction-type-badge.md)
...
- Location: `modules/domains/fintech/TransactionRow.tsx`, named export `TransactionRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `TransactionRowProps` type.
...
- [ ] Exported as `TransactionRow` from the `modules/domains/fintech` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Completed deposit, Mixed statuses)
- [ ] State **hover** implemented: pressed state instead (`active:` classes / Pressable `pressed`)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`border`, `border-strong`, `error`, `error-subtle`, `info`, `info-subtle`, `primary`, `primary-subtle`, …); renders correctly in light and dark
