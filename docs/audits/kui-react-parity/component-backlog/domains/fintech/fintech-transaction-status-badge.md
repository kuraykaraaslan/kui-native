...
> Backlog item · KuiReact id `fintech-transaction-status-badge` · layer `domain` · **Domain — Fintech** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/fintech/transaction/TransactionStatusBadge.tsx` (1 file, 22 LOC) |
...
Parsed from `modules/domains/fintech/transaction/TransactionStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `TransactionStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/fintech/TransactionStatusBadge.tsx`, named export `TransactionStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `TransactionStatusBadgeProps` type.
...
- [ ] Exported as `TransactionStatusBadge` from the `modules/domains/fintech` barrel with its props type
