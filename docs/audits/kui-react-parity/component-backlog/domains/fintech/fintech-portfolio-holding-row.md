...
> Backlog item · KuiReact id `fintech-portfolio-holding-row` · layer `domain` · **Domain — Fintech** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/fintech/portfolio/PortfolioHoldingRow.tsx` (1 file, 97 LOC) |
| Public export | `@/modules/domains/fintech` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Profit / loss rows, No cost basis (no P&L) |
| Composes | CurrencyBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/fintech/portfolio/PortfolioHoldingRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `symbol` | `string` | **yes** | — |  | same |
| `name` | `string` | **yes** | — |  | same |
| `amount` | `number` | **yes** | — |  | same |
| `currency` | `Currency` | **yes** | — |  | same |
| `usdValue` | `number` | **yes** | — |  | same |
| `costBasis` | `number` | no | — |  | same |
| `dayChangePct` | `number` | no | — |  | same |
...
- Location: `modules/domains/fintech/PortfolioHoldingRow.tsx`, named export `PortfolioHoldingRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `PortfolioHoldingRowProps` type.
...
- [ ] Exported as `PortfolioHoldingRow` from the `modules/domains/fintech` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Profit / loss rows, No cost basis (no P&L))
