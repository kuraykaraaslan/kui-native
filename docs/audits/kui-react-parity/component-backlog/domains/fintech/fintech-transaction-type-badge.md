...
> Backlog item · KuiReact id `fintech-transaction-type-badge` · layer `domain` · **Domain — Fintech** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/fintech/transaction/TransactionTypeBadge.tsx` (1 file, 25 LOC) |
| Public export | `@/modules/domains/fintech` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All types, Small size |
...
Parsed from `modules/domains/fintech/transaction/TransactionTypeBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `type` | `TransactionType` | **yes** | — |  | same |
...
- Location: `modules/domains/fintech/TransactionTypeBadge.tsx`, named export `TransactionTypeBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `TransactionTypeBadgeProps` type.
...
- [ ] Exported as `TransactionTypeBadge` from the `modules/domains/fintech` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All types, Small size)
