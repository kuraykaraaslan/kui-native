...
> Backlog item · KuiReact id `forum-topic-row` · layer `domain` · **Domain — Forum** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/forum/topic/TopicRow.tsx` (1 file, 115 LOC) |
| Public export | `@/modules/domains/forum` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Open topic, Pinned and locked variants |
| Composes | TopicStatusBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/forum/topic/TopicRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `topic` | `{ topicId: string; title: string; slug: string; status: TopicStatus; replyCount…` | **yes** | — |  | same |
...
- TopicStatusBadge — missing → [backlog](forum-topic-status-badge.md)
...
- Location: `modules/domains/forum/TopicRow.tsx`, named export `TopicRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `TopicRowProps` type.
...
- [ ] Exported as `TopicRow` from the `modules/domains/forum` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Open topic, Pinned and locked variants)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `surface-base`, `surface-overlay`, `text-primary`, `text-secondary`, `warning`); renders correctly in light and dark
