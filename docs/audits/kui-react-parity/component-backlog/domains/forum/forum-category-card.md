...
> Backlog item · KuiReact id `forum-category-card` · layer `domain` · **Domain — Forum** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/forum/category/ForumCategoryCard.tsx` (1 file, 97 LOC) |
| Public export | `@/modules/domains/forum` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | With stats and activity, Minimal (no stats) |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/forum/category/ForumCategoryCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `category` | `{ categoryId: string; title: string; slug: string; description?: string \| null;…` | **yes** | — |  | same |
...
- Location: `modules/domains/forum/ForumCategoryCard.tsx`, named export `ForumCategoryCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `ForumCategoryCardProps` type.
...
- [ ] Exported as `ForumCategoryCard` from the `modules/domains/forum` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (With stats and activity, Minimal (no stats))
