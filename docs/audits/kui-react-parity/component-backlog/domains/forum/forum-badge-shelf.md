...
> Backlog item · KuiReact id `forum-badge-shelf` · layer `domain` · **Domain — Forum** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/forum/user/BadgeShelf.tsx` (1 file, 96 LOC) |
| Public export | `@/modules/domains/forum` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Six earned badges, Empty state |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/forum/user/BadgeShelf.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `title` | `string` | no | `'Badges'` |  | same |
| `badges` | `ForumBadge[]` | **yes** | — |  | same |
...
- Location: `modules/domains/forum/BadgeShelf.tsx`, named export `BadgeShelf` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `BadgeShelfProps` type.
...
- [ ] Exported as `BadgeShelf` from the `modules/domains/forum` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Six earned badges, Empty state)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`border`, `surface-overlay`, `surface-raised`, `text-primary`, `text-secondary`, `warning`); renders correctly in light and dark
