...
> Backlog item · KuiReact id `forum-user-activity-row` · layer `domain` · **Domain — Forum** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/forum/user/UserActivityRow.tsx` (1 file, 98 LOC) |
| Public export | `@/modules/domains/forum` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All four kinds |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/forum/user/UserActivityRow.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `kind` | `UserActivityKind` | **yes** | — |  | same |
| `topicTitle` | `string` | no | — |  | same |
| `topicHref` | `string` | no | — |  | same |
| `excerpt` | `string` | no | — |  | same |
| `badgeName` | `string` | no | — |  | same |
| `createdAt` | `Date \| string` | **yes** | — |  | same |
...
- Location: `modules/domains/forum/UserActivityRow.tsx`, named export `UserActivityRow` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `UserActivityRowProps` type.
...
- [ ] Exported as `UserActivityRow` from the `modules/domains/forum` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (All four kinds)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `info`, `info-subtle`, `primary`, `primary-subtle`, `success`, `success-subtle`, …); renders correctly in light and dark
