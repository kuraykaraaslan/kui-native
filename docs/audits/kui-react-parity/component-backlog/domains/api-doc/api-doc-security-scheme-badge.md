...
> Backlog item · KuiReact id `api-doc-security-scheme-badge` · layer `domain` · **Domain — API Doc** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/api-doc/SecuritySchemeBadge.tsx` (1 file, 33 LOC) |
...
| Showcase variants | All scheme types, Small size with custom names |
...
Parsed from `modules/domains/api-doc/SecuritySchemeBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/api-doc/SecuritySchemeBadge.tsx`, named export `SecuritySchemeBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `SecuritySchemeBadgeProps` type.
...
- [ ] Exported as `SecuritySchemeBadge` from the `modules/domains/api-doc` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All scheme types, Small size with custom names)
