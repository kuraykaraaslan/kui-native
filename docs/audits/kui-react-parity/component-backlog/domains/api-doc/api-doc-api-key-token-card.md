...
> Backlog item · KuiReact id `api-doc-api-key-token-card` · layer `domain` · **Domain — API Doc** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/api-doc/ApiKeyTokenCard.tsx` (1 file, 166 LOC) |
| Public export | `@/modules/domains/api-doc/ApiKeyTokenCard` — source-only (not exported from a barrel) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Production key, Staging key with revoke |
...
Parsed from `modules/domains/api-doc/ApiKeyTokenCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `name` | `string` | **yes** | — |  | same |
| `token` | `string` | **yes** | — |  | same |
| `masked` | `string` | no | — |  | same |
| `scopes` | `string[]` | no | — |  | same |
| `createdAt` | `Date \| string` | no | — |  | same |
| `lastUsedAt` | `Date \| string \| null` | no | — |  | same |
| `environment` | `'production' \| 'staging' \| 'development'` | no | — |  | same |
| `onRevoke` | `() => void` | no | — |  | same |
...
- Location: `modules/domains/api-doc/ApiKeyTokenCard.tsx`, named export `ApiKeyTokenCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `ApiKeyTokenCardProps` type.
...
- [ ] Exported as `ApiKeyTokenCard` from the `modules/domains/api-doc` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Production key, Staging key with revoke)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `error-subtle`, `primary`, `primary-subtle`, `success`, `surface-base`, …); renders correctly in light and dark
