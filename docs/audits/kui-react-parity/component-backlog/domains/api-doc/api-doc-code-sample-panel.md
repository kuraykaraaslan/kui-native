...
> Backlog item · KuiReact id `api-doc-code-sample-panel` · layer `domain` · **Domain — API Doc** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/api-doc/CodeSamplePanel.tsx` (1 file, 82 LOC) |
...
| Showcase variants | Multi-language sample, Single language |
| Composes | Button (exists as `Button`), ButtonGroup (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/api-doc/CodeSamplePanel.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `samples` | `CodeSample[]` | **yes** | — |  | same |
...
- ButtonGroup — missing → [backlog](../../button-group.md)
...
- Location: `modules/domains/api-doc/CodeSamplePanel.tsx`, named export `CodeSamplePanel` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CodeSamplePanelProps` type.
...
- [ ] Exported as `CodeSamplePanel` from the `modules/domains/api-doc` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Multi-language sample, Single language)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `success`); renders correctly in light and dark
