...
> Backlog item · KuiReact id `api-doc-api-tag-section` · layer `domain` · **Domain — API Doc** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/api-doc/ApiTagSection.tsx` (1 file, 86 LOC) |
...
| Showcase variants | Users tag (expanded), Collapsed by default |
| Composes | EndpointRow (missing), Badge (exists as `Badge`) |
| Used by (registry) | — |
...
Parsed from `modules/domains/api-doc/ApiTagSection.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `tag` | `ApiTag` | **yes** | — |  | same |
| `paths` | `TaggedPath[]` | **yes** | — |  | same |
| `defaultOpen` | `boolean` | no | `true` |  | same |
...
- EndpointRow — missing → [backlog](api-doc-endpoint-row.md)
...
- Location: `modules/domains/api-doc/ApiTagSection.tsx`, named export `ApiTagSection` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `ApiTagSectionProps` type.
...
- [ ] Exported as `ApiTagSection` from the `modules/domains/api-doc` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Users tag (expanded), Collapsed by default)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `surface-base`, `surface-overlay`, `surface-raised`, `text-disabled`, `text-primary`, …); renders correctly in light and dark
