...
> Backlog item · KuiReact id `jobs-company-card` · layer `domain` · **Domain — Jobs** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/jobs/company/CompanyCard.tsx` (1 file, 90 LOC) |
| Public export | `@/modules/domains/jobs` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Verified with job count, Unverified, no count |
| Composes | Avatar (exists as `Avatar` + `AvatarGroup`), Badge (exists as `Badge`) |
| Used by (registry) | — |
...
Parsed from `modules/domains/jobs/company/CompanyCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `company` | `Company` | **yes** | — |  | same |
| `jobCount` | `number` | no | — |  | same |
...
- Location: `modules/domains/jobs/CompanyCard.tsx`, named export `CompanyCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CompanyCardProps` type.
...
- [ ] Exported as `CompanyCard` from the `modules/domains/jobs` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Verified with job count, Unverified, no count)
