...
> Backlog item · KuiReact id `jobs-type-badge` · layer `domain` · **Domain — Jobs** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/jobs/job/JobTypeBadge.tsx` (1 file, 19 LOC) |
| Public export | `@/modules/domains/jobs` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All types |
...
Parsed from `modules/domains/jobs/job/JobTypeBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `type` | `JobType` | **yes** | — |  | same |
...
- Location: `modules/domains/jobs/JobTypeBadge.tsx`, named export `JobTypeBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `JobTypeBadgeProps` type.
...
- [ ] Exported as `JobTypeBadge` from the `modules/domains/jobs` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (All types)
