...
> Backlog item · KuiReact id `reviews-rating-distribution` · layer `domain` · **Domain — Reviews** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/reviews/RatingDistribution.tsx` (1 file, 71 LOC) |
| Public export | `@/modules/domains/reviews` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Typical (4-star skew), Low-rated, counts hidden |
| Composes | — |
| Used by (registry) | reviews-review-summary-card |
...
Parsed from `modules/domains/reviews/RatingDistribution.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `distribution` | `ReviewDistribution` | **yes** | — |  | same |
| `total` | `number` | no | — |  | same |
| `showCounts` | `boolean` | no | `true` |  | same |
...
- Location: `modules/domains/reviews/RatingDistribution.tsx`, named export `RatingDistribution` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `RatingDistributionProps` type.
...
- [ ] Exported as `RatingDistribution` from the `modules/domains/reviews` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Typical (4-star skew), Low-rated, counts hidden)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`warning`, `surface-sunken`, `text-secondary`); renders correctly in light and dark
