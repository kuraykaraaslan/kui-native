...
> Backlog item · KuiReact id `social-marketplace-listing-card` · layer `domain` · **Domain — Social** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/social/marketplace/MarketplaceListingCard.tsx` (1 file, 83 LOC) |
| Public export | `@/modules/domains/social` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Two listings side by side, Single card |
...
Parsed from `modules/domains/social/marketplace/MarketplaceListingCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `listing` | `MarketplaceListingWithSeller` | **yes** | — |  | same |
...
- Location: `modules/domains/social/MarketplaceListingCard.tsx`, named export `MarketplaceListingCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `MarketplaceListingCardProps` type.
...
- [ ] Exported as `MarketplaceListingCard` from the `modules/domains/social` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Two listings side by side, Single card)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `surface-overlay`, `surface-raised`, `surface-sunken`, `text-disabled`, `text-primary`, …); renders correctly in light and dark
