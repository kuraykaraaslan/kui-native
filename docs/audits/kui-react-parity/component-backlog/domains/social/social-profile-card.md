...
> Backlog item · KuiReact id `social-profile-card` · layer `domain` · **Domain — Social** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/social/profile/SocialProfileCard.tsx` (1 file, 154 LOC) |
| Public export | `@/modules/domains/social` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Full profile — other user, Own profile (edit button) |
...
Parsed from `modules/domains/social/profile/SocialProfileCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `user` | `SocialUser` | **yes** | — |  | same |
| `isOwnProfile` | `boolean` | no | `false` |  | same |
| `onFollow` | `(userId: string) => void` | no | — |  | same |
| `onMessage` | `(userId: string) => void` | no | — |  | same |
...
- Location: `modules/domains/social/SocialProfileCard.tsx`, named export `SocialProfileCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `SocialProfileCardProps` type.
...
- [ ] Exported as `SocialProfileCard` from the `modules/domains/social` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Full profile — other user, Own profile (edit button))
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-subtle`, `secondary`, `surface-raised`, `text-primary`, `text-secondary`); renders correctly in light and dark
