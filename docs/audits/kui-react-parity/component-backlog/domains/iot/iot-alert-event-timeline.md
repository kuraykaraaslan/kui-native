...
> Backlog item · KuiReact id `iot-alert-event-timeline` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/alert/AlertEventTimeline.tsx` (1 file, 97 LOC) |
| Public export | `@/modules/domains/iot` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Five event kinds |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/iot/alert/AlertEventTimeline.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `events` | `AlertEvent[]` | **yes** | — |  | same |
...
- Location: `modules/domains/iot/AlertEventTimeline.tsx`, named export `AlertEventTimeline` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `AlertEventTimelineProps` type.
...
- [ ] Exported as `AlertEventTimeline` from the `modules/domains/iot` barrel with its props type
...
- [ ] Showcase demos for all 1 KuiReact variants (Five event kinds)
- [ ] Accessibility mappings in the table above implemented and verified with VoiceOver and TalkBack
- [ ] Uses only semantic tokens (`border`, `error`, `error-subtle`, `info`, `info-subtle`, `success`, `success-subtle`, `surface-overlay`, …); renders correctly in light and dark
