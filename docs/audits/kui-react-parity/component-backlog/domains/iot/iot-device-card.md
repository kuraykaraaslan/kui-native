...
> Backlog item · KuiReact id `iot-device-card` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/device/DeviceCard.tsx` (1 file, 105 LOC) |
| Public export | `@/modules/domains/iot` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Online device, Error + Gateway |
| Composes | DeviceStatusBadge (missing), DeviceTypeBadge (missing) |
| Used by (registry) | — |
...
Parsed from `modules/domains/iot/device/DeviceCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `device` | `Device` | **yes** | — |  | same |
...
- DeviceStatusBadge — missing → [backlog](iot-device-status-badge.md)
- DeviceTypeBadge — missing → [backlog](iot-device-type-badge.md)
...
- Location: `modules/domains/iot/DeviceCard.tsx`, named export `DeviceCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `DeviceCardProps` type.
...
- [ ] Exported as `DeviceCard` from the `modules/domains/iot` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Online device, Error + Gateway)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `primary`, `primary-subtle`, `surface-base`, `surface-overlay`, `text-primary`, `text-secondary`); renders correctly in light and dark
