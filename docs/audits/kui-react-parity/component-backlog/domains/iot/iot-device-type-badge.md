...
> Backlog item · KuiReact id `iot-device-type-badge` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/device/DeviceTypeBadge.tsx` (1 file, 21 LOC) |
...
Parsed from `modules/domains/iot/device/DeviceTypeBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `type` | `DeviceType` | **yes** | — |  | same |
...
- Location: `modules/domains/iot/DeviceTypeBadge.tsx`, named export `DeviceTypeBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `DeviceTypeBadgeProps` type.
...
- [ ] Exported as `DeviceTypeBadge` from the `modules/domains/iot` barrel with its props type
