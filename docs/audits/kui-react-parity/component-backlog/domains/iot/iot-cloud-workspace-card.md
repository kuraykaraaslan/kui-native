...
> Backlog item · KuiReact id `iot-cloud-workspace-card` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/workspace/CloudWorkspaceCard.tsx` (1 file, 131 LOC) |
| Public export | `@/modules/domains/iot` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Active workspace, Suspended workspace |
| Composes | Badge (exists as `Badge`) |
| Used by (registry) | — |
| Usage frequency | imported by 0 production file(s), 1 showcase file(s) |
| Third-party imports | `@fortawesome/react-fontawesome`, `@fortawesome/free-solid-svg-icons` |
| Unit tests | none |
| Interaction flags | keyboard handling, focus-visible ring, animated/transitions, hover styles · 2 raw hex literal(s) in source |

## Required Props

Parsed from `modules/domains/iot/workspace/CloudWorkspaceCard.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `workspace` | `CloudWorkspace` | **yes** | — |  | same |
...
- Location: `modules/domains/iot/CloudWorkspaceCard.tsx`, named export `CloudWorkspaceCard` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `CloudWorkspaceCardProps` type.
...
- [ ] Exported as `CloudWorkspaceCard` from the `modules/domains/iot` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Active workspace, Suspended workspace)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `info`, `info-subtle`, `primary`, `primary-subtle`, `success`, …); renders correctly in light and dark
