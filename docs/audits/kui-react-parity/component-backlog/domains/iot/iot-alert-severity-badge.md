...
> Backlog item · KuiReact id `iot-alert-severity-badge` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/alert/AlertSeverityBadge.tsx` (1 file, 21 LOC) |
| Public export | `@/modules/domains/iot` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All severities, Small |
...
Parsed from `modules/domains/iot/alert/AlertSeverityBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/iot/AlertSeverityBadge.tsx`, named export `AlertSeverityBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `AlertSeverityBadgeProps` type.
...
- [ ] Exported as `AlertSeverityBadge` from the `modules/domains/iot` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All severities, Small)
