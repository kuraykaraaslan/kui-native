...
> Backlog item · KuiReact id `iot-device-status-badge` · layer `domain` · **Domain — IoT** · Priority **Low** · Complexity **Small** · Wave 3 · Fit `adapt` · est. 0.5–1 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/iot/device/DeviceStatusBadge.tsx` (1 file, 22 LOC) |
| Public export | `@/modules/domains/iot` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | All states, Small size |
...
Parsed from `modules/domains/iot/device/DeviceStatusBadge.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `status` | `DeviceStatus` | **yes** | — |  | same |
...
- Location: `modules/domains/iot/DeviceStatusBadge.tsx`, named export `DeviceStatusBadge` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `DeviceStatusBadgeProps` type.
...
- [ ] Exported as `DeviceStatusBadge` from the `modules/domains/iot` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (All states, Small size)
