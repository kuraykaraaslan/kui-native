...
> Backlog item · KuiReact id `landing-announcement-bar` · layer `domain` · **Domain — Landing** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/landing/nav/AnnouncementBar.tsx` (1 file, 71 LOC) |
| Public export | `@/modules/domains/landing` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Default (dismissible), Non-dismissible promo |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/landing/nav/AnnouncementBar.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `message` | `React.ReactNode` | **yes** | — |  | same |
| `cta` | `{ label: string; href: string; external?: boolean }` | no | — |  | same |
| `icon` | `IconDefinition` | no | `faBullhorn` |  | same |
| `dismissible` | `boolean` | no | `true` |  | same |
...
- Location: `modules/domains/landing/AnnouncementBar.tsx`, named export `AnnouncementBar` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `AnnouncementBarProps` type.
...
- [ ] Exported as `AnnouncementBar` from the `modules/domains/landing` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Default (dismissible), Non-dismissible promo)
