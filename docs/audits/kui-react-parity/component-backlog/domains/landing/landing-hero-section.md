...
> Backlog item · KuiReact id `landing-hero-section` · layer `domain` · **Domain — Landing** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/landing/hero/HeroSection.tsx` (1 file, 101 LOC) |
| Public export | `@/modules/domains/landing` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | With image, Text only |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/landing/hero/HeroSection.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/landing/HeroSection.tsx`, named export `HeroSection` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `HeroSectionProps` type.
...
- [ ] Exported as `HeroSection` from the `modules/domains/landing` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (With image, Text only)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `info`, `primary`, `primary-active`, `primary-fg`, `primary-hover`, `surface-base`, …); renders correctly in light and dark
