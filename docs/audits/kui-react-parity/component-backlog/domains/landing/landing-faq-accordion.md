...
> Backlog item · KuiReact id `landing-faq-accordion` · layer `domain` · **Domain — Landing** · Priority **Low** · Complexity **Medium** · Wave 3 · Fit `adapt` · est. 2–3 d
...
| | |
| --- | --- |
| Source | `kui-react/modules/domains/landing/faq/FaqAccordion.tsx` (1 file, 117 LOC) |
| Public export | `@/modules/domains/landing` — source-public (vertical barrel; not in npm package) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Single open, Allow multiple open |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/domains/landing/faq/FaqAccordion.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.
...
- Location: `modules/domains/landing/FaqAccordion.tsx`, named export `FaqAccordion` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `FaqAccordionProps` type.
...
- [ ] Exported as `FaqAccordion` from the `modules/domains/landing` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Single open, Allow multiple open)
