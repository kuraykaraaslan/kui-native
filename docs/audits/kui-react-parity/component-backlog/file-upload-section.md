...
> Backlog item · KuiReact id `file-upload-section` · layer `app` · **Forms** · Priority **Medium** · Complexity **Large** · Wave 3 · Fit `adapt` · est. 5–8 d
...
**Why it matters for KuiNative:** Upload zone with list/progress/preview.

## KuiReact Reference

| | |
| --- | --- |
| Source | `kui-react/modules/app/FileUploadSection/index.tsx` (9 files, 615 LOC) |
| Public export | `@kuraykaraaslan/kui-react/app` — public (npm: root + /app) |
| Registry | yes · status `stable` · since 2026-05 |
| Showcase variants | Basic drop zone, Paste from clipboard |
| Composes | — |
| Used by (registry) | — |
...
Parsed from `modules/app/FileUploadSection/index.tsx`. **Required** = not optional in KuiReact's type. The KuiNative API must keep these names and defaults unless the RN column says otherwise.

| Prop | Type | Required | Default | Notes | RN mapping |
| --- | --- | --- | --- | --- | --- |
| `files` | `FileItem[]` | no | — |  | same |
| `onFilesChange` | `(files: FileItem[]) => void` | no | — |  | same |
| `accept` | `string` | no | — |  | same |
| `multiple` | `boolean` | no | `true` |  | same |
...
| `messages` | `Partial<FileUploadSectionMessages>` | no | — |  | same |
...
**Blocked by (roadmap):** `file-input`, `progress`

**Third-party:** `@fortawesome/react-fontawesome` → @fortawesome/react-native-fontawesome, `@fortawesome/free-solid-svg-icons`

**Suggested RN libraries:** `file-input`, `progress`

## Implementation Notes

Picker button instead of drop zone; list with Progress per file.

- Location: `modules/app/FileUploadSection.tsx`, named export `FileUploadSection` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `FileUploadSectionProps` type.
- Add a showcase entry (`modules/showcase/registry.tsx`) with one demo per KuiReact variant.

## Acceptance Criteria

- [ ] Exported as `FileUploadSection` from the `modules/app` barrel with its props type
...
- [ ] Showcase demos for all 2 KuiReact variants (Basic drop zone, Paste from clipboard)
...
- [ ] Uses only semantic tokens (`border`, `border-focus`, `error`, `error-fg`, `error-subtle`, `primary`, `primary-subtle`, `surface-base`, …); renders correctly in light and dark
