...
> Backlog item · KuiReact id `no-access-state` · layer `app` · **Feedback** · Priority **Medium** · Complexity **Small** · Wave 2 · Fit `direct` · est. 0.5–1 d
> Generated 2026-09-22 from KuiReact source; props/tokens/deps are parsed, not hand-copied. [Back to backlog index](README.md)

## Overview

Permission-denied page state. Exported, not in registry.

**Why it matters for KuiNative:** Exported from KuiReact app layer (EmptyErrorState.tsx) but absent from the registry; permission-denied state.
...
| --- | --- | --- | --- | --- | --- |
| `title` | `string` | no | `'Access denied'` |  | same |
| `description` | `string` | no | `"You don't have permission to…` |  | same |
| `onRequestAccess` | `() => void` | no | — |  | same |
...
- Location: `modules/app/NoAccessState.tsx`, named export `NoAccessState` (same name as KuiReact), re-exported from the layer barrel.
- Styling via `cn()` + semantic token classes; variant/size maps as `Record<Variant, string>` like the existing KuiNative components.
- Props: keep KuiReact names/defaults; spread remaining host props (`...rest`) and forward `ref` so `testID`, `accessibilityHint`, `hitSlop` etc. pass through (current KuiNative components mostly do not).
- Export the `NoAccessStateProps` type.
- Add a showcase entry (`modules/showcase/registry.tsx`) with one demo per KuiReact variant.

## Acceptance Criteria

- [ ] Exported as `NoAccessState` from the `modules/app` barrel with its props type
