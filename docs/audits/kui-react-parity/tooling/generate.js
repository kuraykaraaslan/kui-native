...
const REM_STATUS = {
  ...
  "R-skeleton": ["partial", "Line / Avatar / Text since `048ebed`; Table (`b83d87f`) and DataTable (`da93014`) landed but SkeletonTableRow is still missing"],
...
write("phase-5-roadmap/wave-3-advanced.md", waveDoc(3, "Wave 3 — Advanced", `Nice-to-have: heavy organisms (tables, charts, calendar, media), desktop-web patterns and recommended exceptions. Items with fit \`web-only\` should be closed by adding an exception entry, not by implementation. The Wave 3 components have all landed (TreeView \`9aafffe\` through VideoPlayer \`3d2d0f9\`, so they have left this list) except SkipLink + LiveRegion, which is recorded as an exception candidate.
