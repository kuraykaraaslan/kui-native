...
`KN_REV` makes a refresh reproducible while work keeps landing: the barrel, the test-file list, the test counts and the showcase entry count are read from that commit with `git show` / `git ls-tree` (component sources are read from the checkout). The 2026-09-22 refreshes were run with `KN_REV=0770214` and then `KN_REV=08c1c32`. `generate.js` also reads the barrel's type-export names (`knMeta.typeNames`) for the public-exports table.
